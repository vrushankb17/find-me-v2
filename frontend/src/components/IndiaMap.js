...
const IndiaMap = () => {
  return (
    <div className="map-card">
      ...
      <p className="subtitle" style={{marginBottom: "16px", fontSize: "0.8rem", color: "#888"}}>Monitoring Kumbh Mela Zones</p>
      ...
      <div className="map-inner-container">
        <ComposableMap projection="geoMercator" projectionConfig={{ scale: 800, center: mapCenter }} width={400} height={240} style={{ width: "100%", height: "100%" }}>
            ...
            {({ geographies }) =>
              geographies.map((geo) => (
                <Geography 
                  key={geo.rsmKey} 
                  geography={geo} 
                  fill={geo.properties.name === "India" ? "rgba(16, 185, 129, 0.15)" : "transparent"} 
                  stroke={geo.properties.name === "India" ? "#10b981" : "rgba(255,255,255,0.03)"}
                  strokeWidth={geo.properties.name === "India" ? 1 : 0.5}
                  style={{
                    default: { outline: "none" },
                    hover: { outline: "none", fill: "rgba(16, 185, 129, 0.2)" },
          ...
          <Marker coordinates={[81.8463, 25.4358]}>
            <circle r={8} fill="#fb7185" opacity={0.4}>
